package com.prb.erp.domain.program;
    ...
    public void saveProgram(List<Program> programs) {
        for (Program program : programs) {
            ...
            if (program.isDeleted()) {
                List<String> menuCodes = select().select(qMenu.menuCode).distinct().from(qMenu).where(qMenu.progCd.eq(program.getProgCd())).fetch();
                delete(qAuthGroupMenu).where(qAuthGroupMenu.menuCode.in(menuCodes)).execute();
            ...
            // 저장 처리
                ...
                if (isEmpty(program.getProgCd())) {
                    ...
                    List<String> menuCodes = select().select(qMenu.menuCode).distinct().from(qMenu).where(qMenu.progCd.eq(program.getProgCd())).fetch();
                    List<AuthGroupMenu> authGroupMenuList = select().select(qAuthGroupMenu).from(qAuthGroupMenu).where(qAuthGroupMenu.menuCode.in(menuCodes)).fetch();
